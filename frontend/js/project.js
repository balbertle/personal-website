...
async function loadProjectDetails() {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Project could not be loaded. Status: ${response.status}`);
    }
  ...
  row.className = 'grid grid-cols-1 md:grid-cols-2 gap-4 justify-items-center';

  for (let j = i; j < i + imagesPerRow && j < project.images.length; j++) {
...
while (project.captions && captionIndex < project.captions.length) {
  ...
  imagesContainerEl.innerHTML =
    ...
    descriptionEl.innerText = 'Could not retrieve project details. Please ensure the API is running and the project ID is correct.';
  }
}
